...
== Install
    ...
    gem install tweemux
    tweemux install # puts itself in /usr/local/bin, so guests can run it

== Host Usage

First, create the user on your machine:
...
== Guest Usage
    ...
    gem install tweemux
    tweemux sharpsaw.org 3322 # uses the 'forward' set up from above
